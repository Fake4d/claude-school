...
Guides to Claude — **three comics that build on one another**, plus a special, a
complete command reference and a hands-on guide to setting up your own server
assistant. Every guide comes in **English and German**, each as a PDF and as a
single HTML file, both self-contained (fonts embedded, no external files).
...
| [**Loop Engineering as a comic**](loop-comic/loop-comic-en.pdf) | 9 pages | Special — bonus issue for stage 3: turning `/loop` into a workshop of routines in permanent operation |
...
definitions inside the program itself. As of version 2.1.250 — 62 options,
128 available slash commands, 5 present but switched off. A scheduled job keeps it
